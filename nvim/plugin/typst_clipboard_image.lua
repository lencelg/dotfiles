-- =============================================================================
-- typst-img-paste.nvim (Single File Edition) – Fully Commented
...
-- into a Typst document. The image is saved into an `img/` directory relative
-- to the current file, and a Typst image call `#image("path")` is inserted.
-- =============================================================================

-- Prevent the plugin from being loaded more than once.
if vim.g.loaded_typst_img_paste then return end
vim.g.loaded_typst_img_paste = true
...
M.config = {
  ...
  -- Automatically create a buffer-local keymap for Typst files?
  create_keymap     = true,

  -- Allow overwriting an existing file? If false, a unique name is generated.
  overwrite         = false,

  -- Where to insert the Typst reference:
...
-- Example:
--   require('typst-img-paste').setup({ keymap = "<leader>i", insert_blank_line = false })
function M.setup(opts)
  ...
  if M.config.create_keymap and M.config.keymap then
    ...
    -- Create a new autocmd that sets a buffer-local keymap for every Typst buffer.
    M._autocmd_id = vim.api.nvim_create_autocmd("FileType", {
      pattern = "typst",
      callback = function()
        vim.keymap.set("n", M.config.keymap, "<cmd>PasteTypstImg<CR>", {
          buffer = true,
...
-- =============================================================================
-- LOW‑LEVEL HELPERS (unchanged from Markdown version)
-- =============================================================================
...
local function get_base_dir()
  if vim.bo.filetype ~= "typst" then return nil end
  ...
  vim.notify(
    "Please save the Typst file first (or set `allow_unsaved = true`)",
    vim.log.levels.WARN
  )
  return nil
end
...
function M.paste_image()
  if vim.bo.filetype ~= "typst" then
    vim.notify("This command only works in Typst files.", vim.log.levels.WARN)
    return
  end
  ...
  vim.ui.input({
    prompt = "Typst image filename (e.g. photo or photo.jpg, default: ." .. default_ext .. "): ",
    default = "",
  }, function(input)
    ...
    -- Insert the relative Typst link
    local rel_path = M.config.img_dir .. "/" .. filename
    insert_reference(rel_path)
...
-- =============================================================================
vim.api.nvim_create_user_command("PasteTypstImg", M.paste_image, {})
vim.api.nvim_create_user_command("PasteTypstImgQuick", M.paste_image_quick, {})
